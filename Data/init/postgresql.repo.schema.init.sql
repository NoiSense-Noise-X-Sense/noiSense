SET search_path TO noisense;


CREATE TYPE boundary_type AS ENUM ('AUTONOMOUS_DISTRICT', 'ADMINISTRATIVE_DISTRICT');
CREATE TYPE geometry_format AS ENUM ('GeoJSON', 'WKT');
CREATE TYPE geometry_type AS ENUM ('MultiPolygon', 'Polygon');
CREATE TYPE region_type AS ENUM ('residential_area', 'roads_and_parks', 'industrial_area', 'traditional_markets', 'main_street', 'commercial_area', 'public_facilities', 'parks');

CREATE TABLE comment (
                       comment_id BIGINT GENERATED BY DEFAULT AS IDENTITY,
                       user_id BIGINT NOT NULL,
                       board_id BIGINT NOT NULL,
                       nickname VARCHAR(100),
                       content VARCHAR(300),
                       created_date TIMESTAMP,
                       modified_date TIMESTAMP
);
COMMENT ON TABLE comment IS '게시물 댓글';

CREATE TABLE auth (
                    auth_id BIGINT GENERATED BY DEFAULT AS IDENTITY,
                    user_id BIGINT NOT NULL,
                    token_type VARCHAR(255),
                    access_token VARCHAR(255),
                    refresh_token VARCHAR(255),
                    created_date TIMESTAMP,
                    modified_date TIMESTAMP
);
COMMENT ON TABLE auth IS '사용자 인증';
COMMENT ON COLUMN auth.token_type IS '토큰 유형, 예: Bearer';

CREATE TABLE users (
                     user_id BIGINT GENERATED BY DEFAULT AS IDENTITY,
                     user_nm VARCHAR(100),
                     nickname VARCHAR(100),
                     email VARCHAR(100),
                     role VARCHAR(10),
                     autonomous_district VARCHAR(10),
                     administrative_district VARCHAR(10),
                     created_date TIMESTAMP,
                     modified_date TIMESTAMP
);
COMMENT ON TABLE users IS '사용자';
COMMENT ON COLUMN users.autonomous_district IS '자치구 코드, autonomous_district.code';
COMMENT ON COLUMN users.administrative_district IS '행정동 코드, autonomous_district.code';
COMMENT ON COLUMN users.role IS '사용자 권한';

CREATE TABLE board (
                     board_id BIGINT GENERATED BY DEFAULT AS IDENTITY,
                     user_id BIGINT NOT NULL,
                     nickname VARCHAR(100),
                     title VARCHAR(100),
                     content VARCHAR(500),
                     emotional_score NUMERIC(20),
                     empathy_count NUMERIC(20),
                     view_count NUMERIC(20),
                     autonomous_district VARCHAR(10),
                     administrative_district VARCHAR(10),
                     created_date TIMESTAMP,
                     modified_date TIMESTAMP
);
COMMENT ON TABLE board IS '게시물';
COMMENT ON COLUMN board.emotional_score IS '감정 점수';
COMMENT ON COLUMN board.empathy_count IS '공감 개수';
COMMENT ON COLUMN board.view_count IS '조회 수';
COMMENT ON COLUMN board.autonomous_district IS '자치구 코드, autonomous_district.code';
COMMENT ON COLUMN board.administrative_district IS '행정동 코드, autonomous_district.code';

CREATE TABLE noise_complaints (
                                noise_complaints_id BIGINT GENERATED BY DEFAULT AS IDENTITY,
                                autonomous_district VARCHAR(50),
                                year INTEGER,
                                count INTEGER
);
COMMENT ON TABLE noise_complaints IS '소음 불만';
COMMENT ON COLUMN noise_complaints.autonomous_district IS '자치구 코드, autonomous_district.code';

CREATE TABLE autonomous_district (
                                     code	VARCHAR(10)		NOT NULL,
                                     name_ko	VARCHAR(50)		NOT NULL,
                                     name_en	VARCHAR(50)		NOT NULL
);
COMMENT ON TABLE autonomous_district IS '자치구 코드 목록';
COMMENT ON COLUMN autonomous_district.code IS '자치구 코드';
COMMENT ON COLUMN autonomous_district.name_en IS '자치구 영문명, sensor_data.autonomous_district';
COMMENT ON COLUMN autonomous_district.name_ko IS '자치구 국문명, 검색 조건 및 라벨에 사용';

CREATE TABLE administrative_district (
                                         code	VARCHAR(10)		NOT NULL,
                                         name_ko	VARCHAR(50)		NOT NULL,
                                         name_en	VARCHAR(50)		NOT NULL,
                                         autonomous_district	VARCHAR(10)		NOT NULL
);
COMMENT ON TABLE administrative_district IS '행정동 코드 목록';
COMMENT ON COLUMN administrative_district.code IS '행정동 코드';
COMMENT ON COLUMN administrative_district.autonomous_district IS 'autonomous_district.code';
COMMENT ON COLUMN administrative_district.name_en IS '행정동 영문명, sensor_data.administrative_district';
COMMENT ON COLUMN administrative_district.name_ko IS '행정동 국문명, 검색 조건 및 라벨에 사용';


CREATE TABLE boundary_polygon (
                            boundary_polygon_id BIGINT GENERATED BY DEFAULT AS IDENTITY,
                            autonomous_district     VARCHAR(10),
                            administrative_district VARCHAR(10),
                            boundary_type boundary_type,
                            geometry_format geometry_format DEFAULT 'GeoJSON' NOT NULL,
                            geometry_type geometry_type   DEFAULT 'MultiPolygon' NOT NULL,
                            geometry_coordinate     TEXT NOT NULL,
                            geometry_centroid VARCHAR(100),
                            created_date TIMESTAMP
);
COMMENT ON TABLE boundary_polygon is '경계 좌표 목록';
COMMENT ON COLUMN boundary_polygon.boundary_polygon_id is '경계 식별자';
COMMENT ON COLUMN boundary_polygon.autonomous_district is 'autonomous_district.code';
COMMENT ON COLUMN boundary_polygon.administrative_district is 'administrative_district.code';
COMMENT ON COLUMN boundary_polygon.geometry_format is '데이터 포맷 ENUM.geometry_format';
COMMENT ON COLUMN boundary_polygon.geometry_type is '폴리곤 유형 ENUM.geometry_type';
COMMENT ON COLUMN boundary_polygon.geometry_coordinate is '폴리곤 좌표 JSON ARRAY';
COMMENT ON COLUMN boundary_polygon.geometry_centroid is '폴리곤 중심 좌표';


CREATE TABLE sensor_data (
                             sensor_data_id	BIGINT GENERATED BY DEFAULT AS IDENTITY,
                             sensing_time	TIMESTAMP NOT	NULL,
                             region	region_type NULL,
                             autonomous_district	VARCHAR(100)		NULL,
                             administrative_district	VARCHAR(100)		NULL,
                             max_noise	NUMERIC(5, 2)		NULL,
                             avg_noise	NUMERIC(5, 2)		NULL,
                             min_noise	NUMERIC(5, 2)		NULL,
                             max_temp	NUMERIC(5, 2)		NULL,
                             avg_temp	NUMERIC(5, 2)		NULL,
                             min_temp	NUMERIC(5, 2)		NULL,
                             max_humi	NUMERIC(5, 2)		NULL,
                             avg_humi	NUMERIC(5, 2)		NULL,
                             min_humi	NUMERIC(5, 2)		NULL,
                             batch_time	TIMESTAMP		NULL
);
COMMENT ON TABLE sensor_data IS '일시, 자치구, 행정동 별 센서 데이터';
COMMENT ON COLUMN sensor_data.sensing_time IS 'API의 sensing_time';
COMMENT ON COLUMN sensor_data.region IS 'ENUM.region_type';
COMMENT ON COLUMN sensor_data.autonomous_district IS 'autonomous_district.name_en';
COMMENT ON COLUMN sensor_data.administrative_district IS 'administrative_district.name_en';
COMMENT ON COLUMN sensor_data.max_noise IS '최고 소음(dB)';
COMMENT ON COLUMN sensor_data.avg_noise IS '평균 소음(dB)';
COMMENT ON COLUMN sensor_data.min_noise IS '최저 소음(dB)';
COMMENT ON COLUMN sensor_data.max_temp IS '최고 기온 (섭씨)';
COMMENT ON COLUMN sensor_data.avg_temp IS '평균 기온 (섭씨)';
COMMENT ON COLUMN sensor_data.min_temp IS '최저 기온 (섭씨)';
COMMENT ON COLUMN sensor_data.max_humi IS '최고 습도(퍼센트)';
COMMENT ON COLUMN sensor_data.avg_humi IS '평균 습도(퍼센트)';
COMMENT ON COLUMN sensor_data.min_humi IS '최저 습도(퍼센트)';
COMMENT ON COLUMN sensor_data.batch_time IS 'API로부터 데이터 가져온 일시';


CREATE TABLE board_empathy (
                             board_empathy_id BIGINT GENERATED BY DEFAULT AS IDENTITY,
                             board_id BIGINT NOT NULL,
                             user_id BIGINT NOT NULL,
                             created_date TIMESTAMP,
                             UNIQUE (board_id, user_id)
);
COMMENT ON TABLE board_empathy IS '게시물에 공감한 사용자';

/* constraint */

ALTER TABLE comment ADD CONSTRAINT pk_comment PRIMARY KEY (comment_id);

ALTER TABLE auth ADD CONSTRAINT pk_auth PRIMARY KEY (auth_id, user_id);

ALTER TABLE users ADD CONSTRAINT pk_users PRIMARY KEY (user_id);

ALTER TABLE board ADD CONSTRAINT pk_board PRIMARY KEY (board_id);

ALTER TABLE noise_complaints ADD CONSTRAINT pk_noise_complaints PRIMARY KEY (noise_complaints_id);

ALTER TABLE sensor_data ADD CONSTRAINT pk_sensor_data PRIMARY KEY (sensor_data_id);

ALTER TABLE auth ADD CONSTRAINT fk_users_to_auth_1 FOREIGN KEY (user_id)
  REFERENCES users (user_id);

ALTER TABLE sensor_data ALTER COLUMN "sensing_time" TYPE TIMESTAMP(0);

ALTER TABLE sensor_data ALTER COLUMN "batch_time" TYPE TIMESTAMP(0);

ALTER TABLE board RENAME COLUMN autonomous_district TO autonomous_district_code;
ALTER TABLE board RENAME COLUMN administrative_district TO administrative_district_code;

ALTER TABLE board_empathy ADD CONSTRAINT fk_board_to_board_empathy_1
  FOREIGN KEY (board_id)
    REFERENCES board (board_id)
    ON DELETE CASCADE;

ALTER TABLE board_empathy ADD CONSTRAINT fk_users_to_board_empathy_1
  FOREIGN KEY (user_id)
    REFERENCES users (user_id)
    ON DELETE CASCADE;


ALTER TABLE "autonomous_district" ADD CONSTRAINT pk_autonomous_district PRIMARY KEY (
                                                                                       code
  );

ALTER TABLE administrative_district ADD CONSTRAINT pk_administrative_district PRIMARY KEY (
                                                                                               code
  );

ALTER TABLE boundary_polygon ADD CONSTRAINT pk_boundary_polygon PRIMARY KEY (
                                                                                 boundary_polygon_id
  );


--- Spring Batch Start
-- 🎯 1. Job 인스턴스 (Job 정의 단위)
CREATE TABLE BATCH_JOB_INSTANCE  (
                                   JOB_INSTANCE_ID BIGINT PRIMARY KEY,
                                   VERSION BIGINT,
                                   JOB_NAME VARCHAR(100) NOT NULL,
                                   JOB_KEY VARCHAR(32) NOT NULL,
                                   CONSTRAINT JOB_INST_UN UNIQUE (JOB_NAME, JOB_KEY)
);

-- 🎯 2. Job 실행 이력
CREATE TABLE BATCH_JOB_EXECUTION  (
                                    JOB_EXECUTION_ID BIGINT PRIMARY KEY,
                                    VERSION BIGINT,
                                    JOB_INSTANCE_ID BIGINT NOT NULL,
                                    CREATE_TIME TIMESTAMP NOT NULL,
                                    START_TIME TIMESTAMP,
                                    END_TIME TIMESTAMP,
                                    STATUS VARCHAR(10),
                                    EXIT_CODE VARCHAR(2500),
                                    EXIT_MESSAGE VARCHAR(2500),
                                    LAST_UPDATED TIMESTAMP,
                                    JOB_CONFIGURATION_LOCATION VARCHAR(2500),
                                    CONSTRAINT JOB_INST_EXEC_FK FOREIGN KEY (JOB_INSTANCE_ID)
                                      REFERENCES BATCH_JOB_INSTANCE(JOB_INSTANCE_ID)
);

-- 🎯 3. Job 실행 시 전달된 파라미터 (Spring Batch 5.x 기준 컬럼명)
CREATE TABLE BATCH_JOB_EXECUTION_PARAMS  (
                                           JOB_EXECUTION_ID BIGINT NOT NULL,
                                           PARAMETER_NAME VARCHAR(100) NOT NULL,
                                           PARAMETER_TYPE VARCHAR(50) NOT NULL,
                                           PARAMETER_VALUE VARCHAR(250),
                                           IDENTIFYING CHAR(1) NOT NULL,
                                           CONSTRAINT JOB_EXEC_PARAMS_FK FOREIGN KEY (JOB_EXECUTION_ID)
                                             REFERENCES BATCH_JOB_EXECUTION(JOB_EXECUTION_ID)
);

-- 🎯 4. Step 실행 이력
CREATE TABLE BATCH_STEP_EXECUTION  (
                                     STEP_EXECUTION_ID BIGINT PRIMARY KEY,
                                     VERSION BIGINT NOT NULL,
                                     STEP_NAME VARCHAR(100) NOT NULL,
                                     JOB_EXECUTION_ID BIGINT NOT NULL,
                                     CREATE_TIME TIMESTAMP NOT NULL,
                                     START_TIME TIMESTAMP,
                                     END_TIME TIMESTAMP,
                                     STATUS VARCHAR(10),
                                     COMMIT_COUNT BIGINT,
                                     READ_COUNT BIGINT,
                                     FILTER_COUNT BIGINT,
                                     WRITE_COUNT BIGINT,
                                     READ_SKIP_COUNT BIGINT,
                                     WRITE_SKIP_COUNT BIGINT,
                                     PROCESS_SKIP_COUNT BIGINT,
                                     ROLLBACK_COUNT BIGINT,
                                     EXIT_CODE VARCHAR(2500),
                                     EXIT_MESSAGE VARCHAR(2500),
                                     LAST_UPDATED TIMESTAMP,
                                     CONSTRAINT JOB_EXEC_STEP_FK FOREIGN KEY (JOB_EXECUTION_ID)
                                       REFERENCES BATCH_JOB_EXECUTION(JOB_EXECUTION_ID)
);

-- 🎯 5. Step 실행 컨텍스트 (JSON 등 직렬화 저장)
CREATE TABLE BATCH_STEP_EXECUTION_CONTEXT  (
                                             STEP_EXECUTION_ID BIGINT PRIMARY KEY,
                                             SHORT_CONTEXT VARCHAR(2500) NOT NULL,
                                             SERIALIZED_CONTEXT TEXT,
                                             CONSTRAINT STEP_EXEC_CTX_FK FOREIGN KEY (STEP_EXECUTION_ID)
                                               REFERENCES BATCH_STEP_EXECUTION(STEP_EXECUTION_ID)
);

-- 🎯 6. Job 실행 컨텍스트
CREATE TABLE BATCH_JOB_EXECUTION_CONTEXT  (
                                            JOB_EXECUTION_ID BIGINT PRIMARY KEY,
                                            SHORT_CONTEXT VARCHAR(2500) NOT NULL,
                                            SERIALIZED_CONTEXT TEXT,
                                            CONSTRAINT JOB_EXEC_CTX_FK FOREIGN KEY (JOB_EXECUTION_ID)
                                              REFERENCES BATCH_JOB_EXECUTION(JOB_EXECUTION_ID)
);

-- 🎯 7. 실행결과
CREATE TABLE batch_result_log (
                                batch_result_log_id BIGINT GENERATED BY DEFAULT AS IDENTITY,
                                job_name VARCHAR(100) NOT NULL,
                                message TEXT,
                                created_date TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

-- 🎯 8. 시퀀스
CREATE SEQUENCE BATCH_JOB_SEQ START 1;
CREATE SEQUENCE BATCH_JOB_EXECUTION_SEQ START 1;
CREATE SEQUENCE BATCH_STEP_EXECUTION_SEQ START 1;

--- Spring Batch End

-- 지역 대시보드 테이블 Start

-- 요약 테이블 (자치구별, 집계 단위)
CREATE TABLE dashboard_district_noise_summary (
                                                batch_id VARCHAR(32) NOT NULL,
                                                start_date DATE NULL,
                                                end_date DATE NULL,
                                                autonomous_district_code VARCHAR(10) NOT NULL,
                                                avg_noise NUMERIC(5, 2) NULL,
                                                peak_hour INTEGER NULL,
                                                peak_noise NUMERIC(5, 2) NULL,
                                                calm_hour INTEGER NULL,
                                                calm_noise NUMERIC(5, 2) NULL,
                                                top_keywords JSONB NULL,
                                                PRIMARY KEY (batch_id, autonomous_district_code)
);


-- 시간대별 소음 통계 테이블 (자치구+시간별)
CREATE TABLE dashboard_district_noise_hourly (
                                               batch_id VARCHAR(32) NOT NULL,
                                               start_date DATE NULL,                      -- 통계 시작일 (한 달 평균의 시작일)
                                               end_date DATE NULL,                        -- 통계 종료일 (한 달 평균의 종료일)
                                               autonomous_district_code VARCHAR(10) NOT NULL,  -- 자치구 코드
                                               hour INTEGER NOT NULL,                     -- 시 (0~23)
                                               avg_day NUMERIC(5,2) NULL,                 -- 하루(어제) 평균
                                               avg_week NUMERIC(5,2) NULL,                -- 최근 7일 평균
                                               avg_month NUMERIC(5,2) NULL,               -- 최근 30일 평균
                                               PRIMARY KEY (batch_id, autonomous_district_code, hour)
);


-- 연도별 테이블 (시/구 단위 집계)
CREATE TABLE dashboard_district_noise_yearly (
                                               batch_id VARCHAR(32) NOT NULL,
                                               region_type VARCHAR(10) NOT NULL,
                                               autonomous_district_code VARCHAR(10),
                                               year INTEGER NOT NULL,
                                               avg_noise NUMERIC(5, 2) NULL,
                                               PRIMARY KEY (batch_id, region_type, autonomous_district_code, year)
);

-- 행정동별 소음 테이블 (자치구+행정동별)
CREATE TABLE dashboard_district_noise_zone (
                                             batch_id VARCHAR(32) NOT NULL,
                                             autonomous_district_code VARCHAR(10) NOT NULL,
                                             administrative_district_code VARCHAR(10) NOT NULL,
                                             start_date DATE NULL,
                                             end_date DATE NULL,
                                             avg_noise NUMERIC(5, 2) NULL,
                                             PRIMARY KEY (batch_id, autonomous_district_code, administrative_district_code)
);

-- 지역 대시보드 테이블 End


-- sensor_distrtict_mapping
CREATE TABLE sensor_district_mapping (
                                       sensor_auto_district_en  VARCHAR(50) NOT NULL,
                                       sensor_admin_district_en VARCHAR(50) NOT NULL,
                                       admin_district_code      VARCHAR(10)  NOT NULL
);

COMMENT ON TABLE sensor_district_mapping IS '센서 데이터 테이블과 행정동 코드 매핑';

ALTER TABLE sensor_district_mapping
  ADD CONSTRAINT pk_sensor_district_mapping
    PRIMARY KEY (sensor_auto_district_en, sensor_admin_district_en);

ALTER TABLE sensor_district_mapping
  ADD CONSTRAINT fk_mapping_to_admin_district
    FOREIGN KEY (admin_district_code)
      REFERENCES administrative_district (code);
