...
export default function MainMap() {
  ...
  const currentOverlayCodeRef = useRef<string | null>(null);
    ...
    (async () => {
      ...
      autonomousAreas.forEach(autonomousArea => {
        ...
        kakao.maps.event.addListener(poly, 'mouseover', () => {
          ...
          // 텍스트 삽입
          infoTitleRef.current.textContent = autonomousArea.name ?? '알 수 없음';
          infoNoiseRef.current.textContent = `평균 소음: ${noise.avgNoise.toFixed(1)}dB`;
          infoSubRef.current.textContent = `체감 소음: ${noise.perceivedNoise.toFixed(1)}dB · ${noise.updatedAt}`;
